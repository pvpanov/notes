...
t = 25
d = 20
w = np.tile(np.array([0.5 / (t - 1)] * (t - 1) + [0.5]), d)
# %%
...
def solve_optimization(p0, m, w, c1, c2):
    ...
    if not (len(w) == len(c1) == len(c2) == n):
        raise ValueError(
            "All coefficient arrays (m, w, c1, c2) must have the same length."
        )
    ...
    res = minimize(fun=objective, x0=p_vars0, args=(p0, m, w, c1, c2), method="SLSQP")
...
# %%
p_optimal_0, _ = solve_optimization(0.0, m, w, [0.05] * len(m), [0.05] * len(m))
p_optimal_1, _ = solve_optimization(0.0, m, w, [0.05] * len(m), [0.1] * len(m))
p_optimal_2, _ = solve_optimization(0.0, m, w, [0.1] * len(m), [0.05] * len(m))
# %%
...
plt.plot((m / (2 * w))[:125])
# %%
plt.hist(p_optimal_0, bins=20, alpha=0.5, density=True)
plt.hist((m / (2 * w)), bins=20, alpha=0.5, density=True)
# %%
