...
lgbm_model = lgb.train(
    params, train_data, num_boost_round=100, valid_sets=[train_data, test_data]
)
    ...
    for tree in tree_data:
        ...
        def traverse_tree(node, sample_indices):
            if "leaf_index" in node:
                ...
                if decision_type == "<=":
                    left_indices = sample_indices[
                        X[sample_indices, feature] <= threshold
                    ]
                    right_indices = sample_indices[
                        X[sample_indices, feature] > threshold
                    ]
                else:
                    left_indices = sample_indices[
...
def calculate_crossing_distances(x, threshold, decision_type):
    ...
    avg_distance_below = 0.0
    avg_distance_above = 0.0
...
def prepare_tree_paths(tree, X, mult):
    ...
    while stack:
        ...
        if "leaf_index" in node:
            ...
            tdown, tup = calculate_crossing_distances(
                X[:, feature], threshold, decision_type
            )
            l = [
                feature,
                threshold - tdown * mult,
                threshold,
                threshold + tup * mult,
                decision_type,
                1,
...
# %%
tree_data = prepare_tree_paths(lgbm_model.dump_model()["tree_info"][0], X_test, 0.0)
# %%
...
def fit_ridge(shared_name_X, shape_X, dtype_X, shared_name_y, shape_y, dtype_y, alpha):
...
models = Parallel(n_jobs=3)(
    delayed(fit_ridge)(
        X_shm.name, X.shape, X.dtype, y_shm.name, y.shape, y.dtype, alpha
    )
    for alpha in alphas
)
